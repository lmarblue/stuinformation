...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
  ...
  if(fd==NULL){
   ...
   close(fd);
	...
	if (tno[0] == '*')
	{
		sprintf(sql, "select tno,tname,tage,tsex,location,tel,acno,scm from teacher where fl='1'");
	}
	else
	{
		sprintf(sql, "select tno,tname,tage,tsex,location,tel,acno,scm from teacher where tno='%s' and fl='1'", tno);
	}
