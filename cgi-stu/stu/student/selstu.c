...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
	...
	if (sno[0] == '*')
	{
		sprintf(sql, "select sno,sname,sage,ssex,location,tel,acno,scm from student where fl='1'");
	}
	else
	{
		sprintf(sql, "select sno,sname,sage,ssex,location,tel,acno,scm from student where sno='%s' and fl='1'", sno);
	}
