...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
  ...
  status = cgiFormString("tno",  tno, 32);
	...
	sprintf(sql, "select tno,password from tuser where tno='%s' and password='%s' ",tno,password);
	//fprintf(cgiOut, "%s\n",sql );
	//初始化
	db = mysql_init(NULL);
	...
	if((row = mysql_fetch_row(res)) != NULL){
		fprintf(cgiOut,"<a href=\"/teainfo.html\">点击进入教师页面</a>");
	}else{
		fprintf(cgiOut,"<a href=\"/login_tea.html\">学号或密码错误点击重输</a>");
	}
